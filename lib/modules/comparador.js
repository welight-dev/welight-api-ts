    ...
    function Categoria(obj) {
        ...
        if (_self.filters) {
            ...
            for (var _a = 0, filters_1 = filters; _a < filters_1.length; _a++) {
                var obj_2 = filters_1[_a];
                var filter = new Filter(obj_2.name);
                filter.setOptions(obj_2.options);
        ...
        return _self.products.objects.find(data_params).then(function (page) {
            ...
            if (_self.categories.length === 0) {
                if (data_params.q === '' && Object.keys(params).length === 0) {
                    _self.products_categories.objects.find({ level: 0, limit: 0 }).then(function (page_categories) {
