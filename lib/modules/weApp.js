...
var AppManager = /** @class */ (function () {
    ...
    AppManager.prototype.changeRoute = function (app_route, app_token, kwargs) {
    ...
    AppManager.prototype.createAccountDoadorFundo = function (name, email, activity_id, kwargs) {
        var _this = this;
        if (this._app_token != 'doador_fundo') {
            return Promise.resolve(false);
        }
        this._create_account_loading = true;
        return this._user.createAccountDoadorFundo(name, email, activity_id, this._get_source_login(kwargs)).then(function () {
            return _this._loading_app_profile_member().then(function (auth) {
    ...
    AppManager.prototype.login = function (username, password, kwargs) {
        ...
        return this._user.login(username, password, this._get_source_login(kwargs)).then(function () {
            if (kwargs.hasOwnProperty('next')) {
                ...
                _this.changeRoute(kwargs.next.app_route, kwargs.next.app_token);
            }
            else {
                if (_this._app_token == 'home') {
                    ...
                    _this._auth_loading = false;
                    _this.changeRoute("/", user_app_selected.app_token, { user_app_id: user_app_selected.id });
                }
                else {
                    _this._auth_loading = false;
                    _this.changeRoute('/');
            ...
            if (kwargs.hasOwnProperty('next')) {
                _this._auth_loading = false;
                _this.changeRoute('login', 'home', { next: { app_route: kwargs.next.app_route, app_token: kwargs.next.app_token } });
            }
            else {
                _this._auth_loading = false;
    ...
    AppManager.prototype.quickLogin = function (auth, kwargs) {
        ...
        return this._user.quickLogin(auth, this._get_source_login(kwargs)).then(function () {
            ...
            if (kwargs.hasOwnProperty('app_route')) {
                _this._auth_loading = false;
                _this.changeRoute('login', 'home', { next: { app_route: kwargs.app_route, app_token: _this._app_token } });
            }
            else {
                _this._auth_loading = false;
                _this.changeRoute('login', 'home');
    ...
    AppManager.prototype.authGuardUser = function (current_app_route) {
        ...
        if (this._user.is_authenticated) {
            ...
            return this._user.quickLogin().then(function () {
                ...
                _this._auth_loading = false;
                _this.changeRoute('login', 'home', { next: { app_route: current_app_route, app_token: _this._app_token } });
                return false;
            });
        }
    };
        ...
        return this.authGuardUser(current_app_route).then(function (auth) {
            if (auth) {
                ...
                if (_this._app_profile.initialized) {
                    if (_this.user_has_perm(['member'])) {
                        _this._auth_loading = false;
                        return true;
                    }
                    else {
                        _this._auth_loading = false;
                        _this.changeRoute(_this._route_access_denied);
                        return false;
                    }
                }
                else {
                    _this._auth_loading = true;
                    _this._loading_app_profile_member().then(function (auth) {
                        if (auth) {
                            _this._auth_loading = false;
                            return true;
                        }
