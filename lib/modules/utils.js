...
var StepProgressService = /** @class */ (function () {
    ...
    StepProgressService.prototype._set_menu_itens = function () {
        if (!this._params) {
            this._menu_itens = this._menu_itens_tmp;
        }
        else {
            var itens = [];
            for (var i = 0; i < this._menu_itens_tmp.length; i++) {
                var url = this._menu_itens_tmp[i].url;
                for (var param in this._params) {
                    url = url.replace(":" + param, this._params[param]);
                }
                var new_item = Object.assign({}, this._menu_itens_tmp[i]);
                new_item.url = url;
                itens.push(new_item);
            }
    ...
    Object.defineProperty(StepProgressService.prototype, "menu_itens", {
        get: function () {
    ...
    StepProgressService.prototype.getMenu = function (token) {
        return this._menu_itens.find(function (item) { return item.token === token; });
    };
    StepProgressService.prototype.setParams = function (p) {
        this._params = p;
        this._set_menu_itens();
    };
    return StepProgressService;
}());
exports.StepProgressService = StepProgressService;
