    ...
    User.prototype.setProfile = function (data, kwargs) {
        ...
        if (data &&
            ...
            for (var _i = 0, _a = data.apps; _i < _a.length; _i++) {
                var userapp = _a[_i];
                _self._apps.push(new UserApp(userapp.id, userapp.app_name, userapp.app_token, userapp.app_profile_id, userapp.display_name, userapp.admin, userapp.permissions));
                if (userapp.admin) {
...
var UserAddress = /** @class */ (function (_super) {
    ...
    UserAddress.resource = new api.Tastypie.Resource('we-auth/user/profile', { model: UserAddress });
    return UserAddress;
}(api.Tastypie.Model));
